...
numElements = 1920;
elementSpacing = 6.4e-6;%0.0508/2;
txLocs = [elementSpacing*transpose((-(numElements-1)/2):1:(numElements-1)/2) zeros(numElements,1) zeros(numElements,1)];
...
targRadii = [2e-6 2e-6 2e-6 2e-6 2e-6];
...
doSyntheicWavelengthStuff = true;

%% Settings for plotting interference patterns
zCut = 0;
xCoords = -0.025:0.000125:0.025; %-0.005:0.000025:0.005; %-0.5:0.01:0.5;
yCoords = 0:0.000125:0.05; %0:0.000025:0.01;
...
for w = 1:(numPlots*2)
	if (w <= numPlots)
		txRxFocusingSelector = 2;
	else
		txRxFocusingSelector = 1;
	end
	k = mod(w - 1, numPlots) + 1;
...
% 	g_asdf = g_asdf + (1./abs(g));
	...
	plot_handles_temp.Colorbar.Label.FontSize = 14;
	plot_handles_temp.Legend.FontSize = 10; % 12;

	xlabel('X-Axis Location (mm)', 'FontSize', 14);
	ylabel('Y-Axis Location (mm)', 'FontSize', 14);

	if (txRxFocusingSelector == 1)
		title(['Field Patterns - Right Singular Vector #' num2str(k)], 'FontSize', 16);
		plot_handles_temp.Legend.Location = 'northeast';
	else
		title(['Field Patterns - Left Singular Vector #' num2str(k)], 'FontSize', 16);
		plot_handles_temp.Legend.Location = 'southwest';
	end
end
if (doSyntheicWavelengthStuff)
	sgtitle({'DORT Synthetic Singular Vector Backpropagation', ['\Lambda_{synth} = ' sprintf('%.2f', lambda*1e3) 'mm — \lambda_1 = ' sprintf('%.2f', lambdaSynthConstituentWavelengths(1)*1e9) 'nm, \lambda_2 \approx ' sprintf('%.2f', lambdaSynthConstituentWavelengths(2)*1e9) 'nm'], ''}, 'FontSize', 24, 'FontWeight', 'bold');
else
% 	sgtitle({'DORT Singular Vector Backpropagation', ['Single Wavelength (\lambda = ' sprintf('%.2f', lambda(wavelengthNum)*1e9) 'nm)'], ''}, 'FontSize', 24, 'FontWeight', 'bold');
	sgtitle({'DORT Singular Vector Backpropagation', ['Single Wavelength (\lambda \approx ' sprintf('%.2f', lambda(wavelengthNum)*1e9) 'nm)'], ''}, 'FontSize', 24, 'FontWeight', 'bold');
end
colormap('turbo');
...
% 	pause;
	rng(1);
	d1 = d1 + (50e-6)*(rand(size(d1)) - 0.5);
	rng(2);
	d2 = d2 + (50e-6)*(rand(size(d2)) - 0.5);
...
% 	colormap('turbo');
	...
	scatterPlots{1} = scatter(txLocs(:,1), txLocs(:,2), 50, 'o', 'MarkerFaceColor', 'red', 'MarkerEdgeAlpha', 0, 'DisplayName', 'Tx Element');
	scatterPlots{2} = scatter(rxLocs(:,1), rxLocs(:,2), 50, 'o', 'MarkerFaceColor', 'blue', 'MarkerEdgeAlpha', 0, 'DisplayName', 'Rx Element');
	scatterPlots{3} = scatter(targLocs(:,1), targLocs(:,2), 200, 'd', 'MarkerEdgeColor', 'red', 'MarkerFaceColor', 'white', 'MarkerFaceAlpha', 0, 'MarkerEdgeAlpha', 1, 'LineWidth', 3, 'DisplayName', 'Scatterer', 'HandleVisibility', 'off');
	scatterPlots{4} = scatter(targLocs(:,1), targLocs(:,2), 200, 'd', 'MarkerEdgeColor', 'black', 'MarkerFaceColor', 'white', 'MarkerFaceAlpha', 0, 'MarkerEdgeAlpha', 1, 'LineWidth', 0.5, 'DisplayName', 'Scatterer');
