...
if (doSyntheicWavelengthStuff)
	sgtitle({['\Lambda_{synth} = ' sprintf('%.2f', lambda*1e3) 'mm — \lambda_1 = ' sprintf('%.2f', lambdaSynthConstituentWavelengths(1)*1e9) 'nm, \lambda_2 = ' sprintf('%.2f', lambdaSynthConstituentWavelengths(2)*1e9) 'nm']}, 'FontSize', 24, 'FontWeight', 'bold');
else
% 	sgtitle({'DORT Singular Vector Backpropagation', ['Single Wavelength (\lambda = ' sprintf('%.2f', lambda(wavelengthNum)*1e9) 'nm)'], ''}, 'FontSize', 24, 'FontWeight', 'bold');
	sgtitle({'DORT Singular Vector Backpropagation', ['\lambda \approx ' sprintf('%.2f', lambda(wavelengthNum)*1e9) 'nm'], ''}, 'FontSize', 24, 'FontWeight', 'bold');
end
colormap('turbo');
